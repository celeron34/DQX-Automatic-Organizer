...
async def on_reaction_add(reaction:discord.Reaction, user:discord.Member|discord.User):
    ...
    # 通常参加申請
        ...
        if (reaction.message == ROBIN_GUILD.reclutingMessage and
            ...
            # 参加権チェック
            if not await checkRoleRight(user, reaction.message.channel, {ROBIN_GUILD.MEMBER_ROLE}, '参加権がありません'):
                await reaction.message.remove_reaction(reaction.emoji, user)
                return
...
class FormationTopView(discord.ui.View):
    ...
    async def newPartyButton(self, button:discord.ui.Button, interaction:discord.Interaction):
        ...
        if not await checkRoleRight(user, None, {ROBIN_GUILD.MEMBER_ROLE}, '参加権がありません'):
            return
        user = interaction.user
